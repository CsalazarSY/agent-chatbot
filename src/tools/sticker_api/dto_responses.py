...
from .dto_common import AccessoryOption, ShipToAddress, OrderItemBase
...
class ProductPricing(BaseModel):
    ...
    accessories: Optional[List[AccessoryOption]] = Field([], description="List of available accessories for the product. Maps to Swagger's Accessory model.") # Uses imported AccessoryOption
...
class ProductDetail(BaseModel):
    ...
    accessories: Optional[List[AccessoryOption]] = Field([], description="List of accessories available for this product. Maps to Swagger's Accessory model.") # Uses imported AccessoryOption
...
class OrderItemStatus(OrderItemBase):
    ...
    orderItemIdentifier: str = Field(description="API-defined identifier for the specific order item.") # Re-declare as required
    status: str = Field(description="Textual description of the order item's current status.") # Re-declare as required
...
class OrderDetailResponse(BaseModel):
    ...
    shipTo: ShipToAddress = Field(description="The shipping address associated with the order.") # Uses imported ShipToAddress
    ...
    statusId: Optional[int] = Field(None, description="Numerical status ID of the order (maps to OrderStatusId enum in dto_common). Swagger: 'Orders StatusId'.")
    status: str = Field(description="Textual description of the order's current status.")
    items: List[OrderItemBase] = Field(description="A list of items included in the order. Uses imported OrderItemBase.") # Uses imported OrderItemBase
...
class DesignPreviewResponse(BaseModel):
    ...
    shipTo: Optional[ShipToAddress] = Field(None, description="Optional associated shipping address.") # Uses imported ShipToAddress
    ...
    items: Optional[List[OrderItemBase]] = Field(None, description="List of items associated with the design preview.") # Uses imported OrderItemBase
