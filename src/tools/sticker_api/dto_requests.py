...
from .dto_common import OrderStatusId, AccessoryOption, ShipToAddress, OrderItemBase

# --- Enums --- #
# OrderStatusId moved to dto_common.py

# --- Common Reusable Models --- #
# AccessoryOption moved to dto_common.py
# ShipToAddress moved to dto_common.py
# OrderItemBase moved to dto_common.py
...
class OrderItemCreate(OrderItemBase):
    ...
    itemIdentifier: str = Field(description="Client-defined identifier for the item. Required.") # Re-declare as non-optional for this specific type
    price: float = Field(description="The total price for the specified quantity of this item. Required.") # Re-declare as non-optional
    quantity: int = Field(description="The number of units for this order item. Required.") # Re-declare as non-optional
...
class OrderItemCreateDesign(OrderItemBase):
    ...
    # Ensure required fields from base are handled by caller or have defaults if applicable
    itemIdentifier: str = Field(description="Client-defined identifier for the item. Required.") # Re-declare as non-optional
    price: float = Field(description="The total price for the specified quantity of this item. Required.") # Re-declare as non-optional
    quantity: int = Field(description="The number of units for this order item. Required.") # Re-declare as non-optional
...
class SpecificPriceRequest(BaseModel):
    ...
    countryCode: Optional[str] = Field(None, description="Optional two-letter ISO country code (e.g., 'US', 'CA') for accurate pricing/shipping (defaults to config).")
    ...
    accessoryOptions: Optional[List[AccessoryOption]] = Field(None, description="Optional list of selected accessory options.") # Uses imported AccessoryOption
...
class CreateOrderRequest(BaseModel):
    ...
    orderIdentifier: str = Field(description="Client-defined identifier for the order. Required.")
    orderDate: str = Field(description="Order date in ISO 8601 format string (e.g., 'YYYY-MM-DDTHH:MM:SSZ'). Required.")
    shipTo: ShipToAddress = Field(description="The shipping address details. Required.") # Uses imported ShipToAddress
    orderTotal: float = Field(description="Total amount for the order (e.g., in USD). Required.")
    notes: Optional[str] = Field(None, description="Optional notes for the entire order.")
    items: List[OrderItemCreate] = Field(description="A list of items to include in the order, providing full product details.") # Uses derived OrderItemCreate
...
class CreateOrderFromDesignsRequest(BaseModel):
    ...
    orderTotal: float = Field(description="Total amount for the order (e.g., in USD). Required.")
    notes: Optional[str] = Field(None, description="Optional notes for the entire order.")
    items: List[OrderItemCreateDesign] = Field(description="A list of items to include in the order, referencing existing design IDs.") # Uses derived OrderItemCreateDesign
...
class ListOrdersByStatusRequest(BaseModel):
    ...
    status: OrderStatusId = Field(description="The status ID (from OrderStatusId enum) to filter orders by. Required.") # Uses imported OrderStatusId
