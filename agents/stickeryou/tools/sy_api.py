...
async def _make_sy_api_request(
    ...
    """
    if not config.API_BASE_URL: return f"{API_ERROR_PREFIX} Configuration Error - Missing API Base URL."

    auth_token = config.SY_API_AUTH_TOKEN_DYNAMIC or config.API_AUTH_TOKEN # Prioritize dynamic
    if not auth_token: return f"{API_ERROR_PREFIX} Configuration Error - No SY API auth token available (static or dynamic)."
    ...
    while retry_count <= max_retries:
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 401: return f"{API_ERROR_PREFIX} Unauthorized (401) after retries."
                elif response.status_code == 400: return f"{API_ERROR_PREFIX} Bad Request (400). Check parameters/payload."
                elif response.status_code == 404: return f"{API_ERROR_PREFIX} Not Found (404)."
...
async def sy_create_design(
    ...
    if isinstance(result, dict):
        ...
        return f"{API_ERROR_PREFIX} Unexpected successful response type: {type(result)}. Expected Dict."
...
async def sy_list_orders_by_status_get(
    ...
    if isinstance(result, list):
        ...
        return f"{API_ERROR_PREFIX} Unexpected successful response type: {type(result)}. Expected List."
...
async def sy_get_order_details(
    ...
    if isinstance(result, dict):
        ...
        if "Not Found (404)" in result: return f"{API_ERROR_PREFIX} Order not found (404)." # More specific error
        return result
    else:
        return f"{API_ERROR_PREFIX} Unexpected successful response type: {type(result)}. Expected Dict."
...
async def sy_cancel_order(
    ...
    # Example: elif isinstance(result, str) and not result.startswith(API_ERROR_PREFIX): return {"status": "success", "message": result}
         ...
         if "Not Found (404)" in result: return f"{API_ERROR_PREFIX} Order not found (404)."
         return result
    else:
        return f"{API_ERROR_PREFIX} Unexpected successful response type: {type(result)}. Expected Dict (or simple confirmation)."
...
async def sy_get_order_tracking(
    ...
    if isinstance(result, dict):
        ...
        if "Not Found (404)" in result: return f"{API_ERROR_PREFIX} Tracking not available or order not found (404)."
        return result
    else:
        return f"{API_ERROR_PREFIX} Unexpected successful response type: {type(result)}. Expected Dict."
...
async def sy_list_products(
    ...
    if isinstance(result, (dict, list)):
        ...
        return f"{API_ERROR_PREFIX} Unexpected successful response type: {type(result)}. Expected Dict or List."
...
async def sy_verify_login(
    ...
    if isinstance(result, dict):
        ...
        elif "timed out" in result: return f"{API_ERROR_PREFIX} Verification request timed out."
        elif "Network or connection error" in result: return f"{API_ERROR_PREFIX} Network error during verification."
